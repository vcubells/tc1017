...
//  t8c1e3
//
//  Created by Vicente Cubells Nonell on 19/06/14.
...
void generaCSV(int pan, int tortilla, int leche, int huevos, int queso);
...
int main(int argc, const char * argv[])
{
    ...
    generaCSV(pan, tortilla, leche, huevos, queso);
...
void generaCSV(int pan, int tortilla, int leche, int huevos, int queso)
{
    ...
    FILE * file = fopen("/Users/vcubells/Desktop/supermercado.csv", "w");
    
    // Escribe el título de las columnas
    fprintf(file, "%s,%s,%s\n", "Producto", "Cantidad", "Unidad");
    
    // Escribir la información en el archivo
    fprintf(file, "%s,%d,%s\n", "Pan", pan, "bolsas");
    fprintf(file, "%s,%d,%s\n", "Tortilla", tortilla, "Kgs");
    fprintf(file, "%s,%d,%s\n", "Leche", leche, "Litros");
    fprintf(file, "%s,%d,%s\n", "Huevos", huevos, "Cartones");
    fprintf(file, "%s,%d,%s\n", "Queso", queso, "Kgs");
